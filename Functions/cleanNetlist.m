function [ netData , types ] = cleanNetlist( netlistPath )
    if isequal( netlistPath , '' )
        errordlg( 'Please select a valid netlist file.' , 'Error' );
        error( 'error' );
    end
    netData = readmatrix( netlistPath , 'OutputType' , 'string' , 'Delimiter' , " " , 'Range' , 2 );
